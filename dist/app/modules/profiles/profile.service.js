...
const updateProfile = (userId, payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let existingProfile = yield prisma_1.default.profile.findUnique({
        where: { userId },
    });
    // If profile doesn't exist, create a new one
    if (!existingProfile) {
        existingProfile = yield prisma_1.default.profile.create({
            data: Object.assign({ userId }, payload),
        });
    }
    else {
        // Update the existing profile
        existingProfile = yield prisma_1.default.profile.update({
            where: { userId },
            data: payload,
        });
    }
